...
function InsertPage($ID, $title, $lingua, $dir, $descrizione="", $conn){
...
class Template
{
    ...
    public static function save($data = [], $filename = 'post.php', $lingua, $idlingua, $conn)
    {
        InsertPage((int)$data[6], $data[0], $idlingua, '/PrjWiki/BraveFrontierWiki/' . $filename . '/' . $lingua, $data[1], $conn);
    }
}
...
for ($i = 1; $i <= 1000; $i++) {
    ...
    Template::save($data, "$data[0]", "Italiano", 1, $conn);
}
...
for ($i = 0; $i < 100; $i++) {
    ...
    Template::save($data, "$data[0]", "English", 2, $conn);
}
...
for ($i = 0; $i <10; $i++) {
    ...
    $data = array("Pagina_di_prova_n$id", $desc, "Paragrafo di prova n$i", $text, "../../IMG/Unit_ills_thum_" . $img . ".png", "../../IMG/Unit_ills_full_" . $img . ".png", $i + $firstid);
    Template::save($data, "$data[0]", "Francois", 3, $conn);
}